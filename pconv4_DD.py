...
class PConvUNet():
    def __init__(self, image_size = 256, batch_size = 1, lr = 2e-4, m = 64):
        ...
        self.img_folder = "combinedBackFore/"
        self.msk_folder = "mask4channel/mask4channel/"
        ...
        for e in range(epochs):
            ...
            for i in range(self.cache_size):
                ...
                if i % plot_interval == 0 and plot_progress:
                    ...
                    self.plot_example(x[0] * y[0], x[0], x[0][:,:,:3] * y[0][:,:,:3], x[0][:,:,3] * y[0][:,:,3], gen_output[0][:,:,:3], gen_output[0][:,:,3], e, i)                   #inp, tar, genrgb, gendepth, comp
        return loss_values
    ...
    def load_data(self):
        ...
        while(len(self.images) < self.cache_size):
            ...
            image = np.array(img.open(self.img_folder + self.image_files[r]), dtype = np.float32)
            h, w, _ = image.shape
        ...
        while(len(self.masks) < self.cache_size):
            ...
            mask = np.array(img.open(self.msk_folder + self.mask_files[r]), dtype = np.float32)
            h, w, _ = mask.shape
            if h >= self.image_size and w >= self.image_size:
    ...
    def gen_loss(self, dis_output_real, dis_output_fake, dis_output_comp, dd2_output_real, dd2_output_fake,  target,\
        ...
        # perceptual loss for dd2, only using fake and real not comp
            l += tf.reduce_mean(tf.math.abs(dd2_output_real[p] - dd2_output_fake[p])) * weights[2]
    ...
    def plot_example(self, inpRGBD, targetRGBD, inputRGB, inputDepth, genrgb, gendepth,e, i):
        # input RGBD, target RGBD, input RGB, input D, Generated RGB, Generated Depth
        plt.close()
        ...
        plt.savefig(f'res4_08122022_images/epoch_{e}_iter_{i:05d}.png', bbox_inches='tight')
...
model = PConvUNet(image_size = 256, batch_size = 6, lr = 0.001)


loss_arr = model.train(epochs = 100, plot_progress = True)

np.save('FourChannel2D_depth_combinedBackFore', loss_arr)
...
issues, but also helps us to achieve faster convergence
